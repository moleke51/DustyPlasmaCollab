...
def SOML_surface_potential_finder(Theta,mu,z,upsilon): 
    ...
    x = (Theta*s_1)/(s_2) - realLambertW(((mu*z*np.sqrt(Theta))/(s_2))*np.exp((Theta*s_1)/(s_2)))
    return x
...
def SOML_surface_potential_finder_abs(Theta,mu,z,upsilon): 
    x = np.absolute(SOML_surface_potential_finder(Theta,mu,z,upsilon))
    return x
...
def SMOML_surface_potential_finder(Theta,mu,z,gamma,upsilon):
    s_1 = ((np.sqrt(np.pi))*(1+2*(upsilon**2))*sps.erf(upsilon))/(4*upsilon) + 0.5*np.exp(-(upsilon**2))
    s_2 = (np.sqrt(np.pi)*sps.erf(upsilon))/(2*upsilon)
    x = (Theta*s_1)/(s_2) - realLambertW(((np.sqrt(2*np.pi*Theta*(1+gamma*Theta)))/(s_2))*np.exp((Theta*s_1)/(s_2))) + 0.5*np.log((2*np.pi*(1+gamma*Theta))/((z**2)*(mu)**2))
    return x
...
def SMOML_surface_potential_finder_abs(Theta,mu,z,gamma,upsilon):
    x = np.absolute(SMOML_surface_potential_finder(Theta,mu,z,gamma,upsilon))
    return x
...
def potential_finder(Theta,mu,z,alpha,upsilon):
    ...
    alpha_TS = 50
    Phi_SMOML = SMOML_surface_potential_finder_abs(Theta,mu,z,5/3,upsilon) #Set gamma = 5/3
