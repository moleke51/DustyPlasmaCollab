...
def potential_finder(Theta,mu,z,gamma,upsilon):
    ...
    Phi = (Theta*s_1)/(s_2) - realLambertW(((np.sqrt(2*np.pi*Theta*(1+gamma*Theta)))/(s_2))*np.exp((Theta*s_1)/(s_2))) + 0.5*np.log((2*np.pi*(1+gamma*Theta))/((z**2)*(mu)**2))
