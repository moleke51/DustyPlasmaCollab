...
for i in range(0, len(J)):
    if J[i] < 1e-6 :
        ...
        res[i] = 0.0001
for i in range(0,len(J)):
    alpha[i],Phi_a[i] = ABR_potential_solver(mu,alpha_lim,z,J[i], i ,ABR_gamma,res)

with open(species + str(z) +"data.txt","w") as f:
    for (distance,potential) in zip(alpha,Phi_a):
        f.write(f"{distance},{potential}\n")

plt.plot(alpha, Phi_a, label = species + 'plasma')
plt.title('ABR')
