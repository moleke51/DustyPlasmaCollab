...
def is_valid(name, requirements, var_counter, units=None):
    ...
    try:
        ...
        user_input = eval_input(
            (user_input.replace("eV", "ev").replace("ev", "*11594")).translate(
                str.maketrans(prefixes)
            )
        )
        errors = []
...
class Model:
    ...
    def priority(self):
        return getattr(__import__(self._name), "priority")(
            self._Theta, self._alpha, self._upsilon
        )

    def potential_finder(self):
        return getattr(__import__(self._name), "potential_finder")(
            self._Theta, self._mu, self._z, self._alpha, self._upsilon
        )
...
def get_Norm_Potential(
    ...
    modellist = modelpicker(
        "DustyPlasmaCollab/Plasma_code/Models/", Theta, mu, z, alpha, upsilon
    )
    priority = 0
    ...
    if variable_counter == 0:
        ...
        if modellist[modelindex].get_name() == "ABR" and previous_model == "ABR":
            ...
            return (
                modellist[modelindex].potential_finder(),
                modellist[modelindex].get_name(),
                modellist[modelindex].get_colour(),
            )
...
def grapher(input_list, variable_index, Dimensionless, variable_counter=1):
    ...
    if Dimensionless == False:
        ...
        alpha = np.array(
            input_list[5]
            / np.sqrt((epsilon_0 * k_B * input_list[1]) / (input_list[2] * e ** 2))
        )
        upsilon = np.array(
            input_list[6] / np.sqrt(2 * k_B * input_list[0] / input_list[4])
        )
        for i in range(len(input_list[variable_index])):
            Phi, mod_name, colour = get_Norm_Potential(
                Theta[i],
                mu[i],
                z[i],
                alpha[i],
        ...
        for i in range(len(mod_list)):
            plt.plot(
                input_list[variable_index][mod_range[i] : mod_range[i + 1] - 1],
                Phi[mod_range[i] : mod_range[i + 1] - 1],
                color=colour_list[i],
                label=mod_list[i],
            )
        plt.title(
            f"Variation of dust surface potential with {title_list[variable_index]}"
        )
        ...
        title_list = [
            ...
            "mu",
            "relative ion charge",
            "normalised dust radius",
            "normalised plasma flow speed",
        ]
        ...
        plt.title(
            f"Variation of normalised dust surface potential with {title_list[variable_index]}"
        )
...
if choice == "y":
    ...
    z, variable_counter = is_valid(
        "relative ion charge", [GreaterThan(0), IsInteger()], variable_counter
    )
    Theta, variable_counter = is_valid(
        "Theta", [GreaterThanEqualTo(0)], variable_counter
    )
    alpha, variable_counter = is_valid(
        "alpha", [GreaterThanEqualTo(0)], variable_counter
    )
    ...
    z, place_holder = is_valid(
        "relative ion charge",
        [GreaterThan(0), IsInteger(), LessThanEqualTo(z_max)],
        -1,
        "1.60*10^-19 C",
    )
    T_i, variable_counter = is_valid(
        "ion temperature", [GreaterThan(0)], variable_counter, "Kelvin"
    )
    T_e, variable_counter = is_valid(
        "electron temperature", [GreaterThan(0)], variable_counter, "Kelvin"
    )
    n_e, variable_counter = is_valid(
        "electron density",
        [GreaterThan(0)],
        variable_counter,
        "electrons per meter cubed",
    )
    a, variable_counter = is_valid(
        "dust radius", [GreaterThanEqualTo(0)], variable_counter, "meters"
    )
    v, variable_counter = is_valid(
        "plasma flow speed",
